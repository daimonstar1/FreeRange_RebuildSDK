...
public class ResourceChecker : EditorWindow
{
    ...
    bool ShowFromMeshRenderers = true;
    bool ShowFromParticleRenderers = true;
    bool ShowFromFontRenderers = true;
    ...
    [MenuItem("FRG/Editor/Resource Analyzer")]
    static void Init()
    {
        var window = EditorWindow.GetWindow<ResourceChecker>();
    ...
    void OnGUI()
    {
        ...
        ShowFromMeshRenderers = GUILayout.Toggle(ShowFromMeshRenderers, "Show from Mesh renderers");
        ShowFromParticleRenderers = GUILayout.Toggle(ShowFromParticleRenderers, "Show from Particle renderers");
        ShowFromFontRenderers = GUILayout.Toggle(ShowFromFontRenderers, "Show from Font renderers");
        ...
        switch (format)
        {
			...
			*/
            ...
            case TextureFormat.ETC_RGB4://	 ETC (GLES2.0) 4 bits/pixel compressed RGB texture format.
                return 4;
#if UNITY_2018_1_OR_NEWER // pixel formats changed in 2018
    ...
    int CalculateTextureSizeBytes(Texture tTexture)
    {
        ...
        if (tTexture is Cubemap)
        {
            ...
            float tSize = tWidth * tHeight * 6 * bitsPerPixel / 8;
            return Mathf.CeilToInt(tSize);
        }
        return 0;
    }
    ...
    void CheckResources()
    {
        ...
        {
            if (!ShowFromMeshRenderers && (renderer is MeshRenderer || renderer is SkinnedMeshRenderer))
                continue;
            if (!ShowFromParticleRenderers && (renderer is ParticleSystemRenderer))
                continue;
        ...
        if (IncludeGuiElements)
        {
            ...
            {
                if (!ShowFromFontRenderers && graphic is Text)
                    continue;
                //if (graphic is FRG.Core.UI.UIInvisibleGraphic)
                //    continue;
