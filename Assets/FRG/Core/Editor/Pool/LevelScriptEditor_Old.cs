...
namespace FRG.Core {
    [InitializeOnLoad]
    [UnityEditor.CustomEditor(typeof(PoolObjectSpawner_Old))]
    public class LevelScriptEditor_Old : Editor {
        ...
        public override void OnInspectorGUI() {
            base.OnInspectorGUI();
            PoolObjectSpawner_Old tar = target as PoolObjectSpawner_Old;
            
            if(tar != null && tar.ElementPrefab.IsValid) {
        ...
        static void HierarchyItemOnGUI( int instanceID, Rect selectionRect ) {
            ...
            if( hasPoolObjectSpawner.ContainsKey( instanceID ) ) {
                ...
                if( go == null ) return;
                spawner = go.GetComponent<PoolObjectSpawner_Old>();
            } else {
                go = EditorUtility.InstanceIDToObject( instanceID ) as GameObject;
                if( go == null ) return;
                spawner = go.GetComponent<PoolObjectSpawner_Old>();
            ...
            if( spawner != null && spawner.ElementPrefab.IsValid ) {
        ...
        public void DoInspectorGuiOnSpawnerNode(SpawnerNode node) {
            ...
            try {
                ...
                } else if(!node.spawner.ElementPrefab.IsValid) {
        ...
        public List<PoolObjectSpawner_Old> GetDirectChildSpawners(Transform trans) {

            List<PoolObjectSpawner_Old> spawners = new List<PoolObjectSpawner_Old>();
            for(int i=0;i<trans.childCount;i++) {
                Transform child = trans.GetChild(i);
                if(child == trans) continue;
                PoolObjectSpawner_Old spawner = child.gameObject.GetComponent<PoolObjectSpawner_Old>();
                if(spawner != null) {
                    spawners.Add(spawner);
                    continue;
                }
                List<PoolObjectSpawner_Old> spawnersInChild = GetDirectChildSpawners(child);
        ...
        public SpawnerNode GetSpawnerTree(PoolObjectSpawner_Old spawner, SpawnerNode parent, int depth) {
            ...
            List<PoolObjectSpawner_Old> subSpawners = GetDirectChildSpawners(spawner.transform);
        ...
        public class SpawnerNode {
            ...
            [SerializeField] public PoolObjectSpawner_Old spawner = null;
