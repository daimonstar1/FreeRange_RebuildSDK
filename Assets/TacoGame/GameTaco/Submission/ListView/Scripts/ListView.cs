...
namespace Endgame
{
	...
	public partial class ListView : MonoBehaviour
	{
		...
		private Vector2 columnsPanelSize = Vector2.zero;
		...
		private void UpdateScrollBars()
		{
			...
			if (this.activeColumnPanels.Count > 0)
			{
				...
				{
					columnsPanelRightOffset = 0;// verticalScrollBarWidth;
				...
				this.columnsPanelSize = new Vector2(columnsWidth, itemsPlusHeadingHeight);
		...
		private void UpdateMouseWheelScrolling()
		{
			...
			if (this.ContainsPoint(Input.mousePosition))
			{
				...
				Scrollbar verticalScrollbar = this.verticalScrollBar.GetComponent<Scrollbar>();

				// [GD 5/10/15] Instead of scrolling proportionally to the rect size, scroll a fixed pixel amount.
				const int pixelsToScrollPerTick = 35;
				verticalScroll *= (pixelsToScrollPerTick * 10f) / this.columnsPanelSize.y;

				// [NK] Workaround for the "bottom to top" scrollbar bug.
				//verticalScrollbar.value -= verticalScroll;
				verticalScrollbar.value += verticalScroll;
			}
		}
      ...
      //Debug.Log("==== " + text.text);
      //Debug.Log("==== " + itemButton.ListViewSubItem);
      if (itemButton.ListViewSubItem != null)
        text.text = itemButton.ListViewSubItem.Text;
      else {
        Debug.Log ("itemButton.ListViewSubItem is null!");
      }
		...
		private void UpdateScrollBarsAndColumns()
		{
#if ENABLE_PROFILING
		...
		private void LayOutColumnPanels()
		{
			float xPosition = 0;
		...
		private void AddColumnToHierarchy(ColumnHeader columnHeader)
		{
			...
			if (columnHeader.ColumnPanelInHierarchy) {
				Image headerSortIcon = columnHeader.ColumnPanelInHierarchy.Button.transform.Find ("Text/SortImage").GetComponent<Image> ();//getcomponent<image>

				if (headerSortIcon != null) {
					if(TacoManager.columnSortName == columnHeader.Text) {
						headerSortIcon.color = new Color32 (255, 255, 255, 255);
						headerSortIcon.sprite = TacoConfig.sortSprites [TacoManager.columnSortType];
					} else {
						headerSortIcon.color = new Color32 (0, 0, 0, 0);
					}
				}
			}
		...
		private void AddItemToHierarchy(ListViewItem item)
		{
			int index = 0;
				...
				foreach (Transform columnPanel in this.activeColumnPanels)
				{
					...
					if (listViewItem.ImageList != null)
					{
						...
						if (this.smallImageList.WasImageSizeSet) {
							...
							//imageSizeDelta.y = this.smallImageList.ImageSize.y;
							...
							if (subItem.Width != 0) {
								...
								imageSizeDelta.x = subItem.Width;
							...
							if (subItem.Height != 0) {
								...
								//imageSizeDelta.y = subItem.Height;
					...
					bool displayText = !string.IsNullOrEmpty (text.text);
					...
					if (displayImage && displayText) {
						...
						textTransform.localPosition = new Vector3 (textTransform.localPosition.x, -150);
					}
						...
						offsetMin.x = horizontalMargin;
						textTransform.offsetMin = offsetMin;
						textTransform.localPosition = new Vector3 (textTransform.localPosition.x, -100);
						imageTransform.localPosition = textTransform.localPosition;
					}
					else
					{
					...
					customControlParentTransform = itemButtonTransform.Find("CustomControlParent");
		...
		private void OnSelectionClearedInternal()
		{
			if (this.SelectedIndices.Count > 0)
			{
				int selectedIndex = this.SelectedIndices[0];
		...
		private void OnSelectionRemovedInternal(int index)
		{
		...
		private void OnSelectionAddedInternal(int index, bool updateGameObjects)
		{
			int selectedIndex = this.SelectedIndices[0];
		...
		private void SelectItem(int index)
		{
			...
			// [NK 10/08/2015] Replaced reflection with a private interface,
			// [NK 10/08/2015] as reflection isn't available on all Unity platforms.

			// Clear the selected items.
			ISelectedListViewItemCollectionInternals selectedItemsInternals =
				this.selectedItems;
			selectedItemsInternals.ClearInternal();

			// Add the selected item.
			ListViewItem itemToAdd = this.Items[index];
			selectedItemsInternals.AddInternal(itemToAdd);
		}
		...
		{
			// [NK 10/08/2015] Removed a use of reflection here, as
			// [NK 10/08/2015] it is not supported on all Unity platforms.
			// [NK 10/08/2015] Unfortunately this means that SetSelectedFlagInternal
			// [NK 10/08/2015] can now be called by client code.
			ListViewItem itemToSelect = this.Items[index];
			itemToSelect.SetSelectedFlagInternal(true);
		}
		...
		private void DeselectItemProperty(int index)
		{
			...
			// [NK 10/08/2015] can now be called by client code.
			ListViewItem itemToSelect = this.Items[index];
			itemToSelect.SetSelectedFlagInternal(false);
		}
    ...
    private void UpdateBackgroundColor(){
      ...
      for (int i = 0; i < this.Items.Count; i++) {
        ...
        this.items [i].UpdateBackgroundColor (backgroundColor);
      }
    }
		...
		private void SetColumnPanelWidth(ColumnPanel columnPanel)
		{
		...
		public void ScrollToEnsureItemVisible(int index)
		{
		...
		{
			if ((this.GotFocus != null) && value)
			{
				this.GotFocus(this, new System.EventArgs());
			}

			if ((this.LostFocus != null) && !value)
			{
				this.LostFocus(this, new System.EventArgs());
			}
			...
			if (value)
			{
				if (this.selectedIndices.Count == 0)
				{
					// [GD 12/1/16] Do we need to automatically select an item? This prevents us
					// [GD 12/1/16] from unselecting all items in code.
					//this.selectedIndices.Add(0);
				}
			}
