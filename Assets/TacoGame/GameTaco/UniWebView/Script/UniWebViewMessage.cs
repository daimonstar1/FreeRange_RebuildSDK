...
public struct UniWebViewMessage {
    ...
    public UniWebViewMessage(string rawMessage): this() {
        ...
        this.RawMessage = UnityWebRequest.UnEscapeURL(rawMessage);
        ...
        if (schemeSplit.Length >= 2) {
            ...
            this.Path = UnityWebRequest.UnEscapeURL(split[0].TrimEnd('/'));
            this.Args = new Dictionary<string, string>();
            if (split.Length > 1) {
                foreach (string pair in split[1].Split("&"[0])) {
                    string[] elems = pair.Split("="[0]);
                    if (elems.Length > 1) {
                        var key = UnityWebRequest.UnEscapeURL(elems[0]);
                        if (Args.ContainsKey(key)) {
                            var existingValue = Args[key];
                            Args[key] = existingValue + "," + UnityWebRequest.UnEscapeURL(elems[1]);
                        } else {
                            Args[key] = UnityWebRequest.UnEscapeURL(elems[1]);
                        }
