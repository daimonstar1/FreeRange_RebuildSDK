...
namespace GameTaco {
	public class PublicTournamentsListFake : BaseListBehavior {

		public static PublicTournamentsListFake Instance;
			...
			if (this.ListView.SelectedIndices.Count > 0) {
				...
				if (Items.Count > index) {
					...
					if(t.status == "cancelled"){
						...
						TournamentManagerFake.Instance.TappedGameFromList(t);
					}else{
						TournamentManagerFake.Instance.TappedJoinFromList (t);
					}
		...
		protected override void CreateSubObjects(ListViewItem item, string columnName) {
			...
			if (columnName == TacoConfig.TournamentActionColumn) {
				...
				} else if (TacoManager.User != null && System.Array.IndexOf (t.memberIds, TacoManager.User.userId) > -1) {
					// and play now
