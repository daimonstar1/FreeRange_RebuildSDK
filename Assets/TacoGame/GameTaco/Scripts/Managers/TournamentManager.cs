...
namespace GameTaco {
	public class TournamentManager : MonoBehaviour {
		...
		private GameObject fundsRunningLowPanel;
		private GameObject depositDisablePanel;
		...
		public void Open() {
			...
			if (TacoSetup.Instance.TournamentCategory == TournamentCate.Cash) {
				if (!cashToggle.isOn) cashToggle.isOn = true;
				else cashToggle.onValueChanged.Invoke(true);
			}
			else {
				if (!tacoToggle.isOn) tacoToggle.isOn = true;
				else tacoToggle.onValueChanged.Invoke(true);
			}
			SetMoneytext();
		}
		...
		public void SetRunningLowFundText(string text, string tourType)
		{
			if(tourType == "Cash")
			{
				GameObject.Find("TacoGame/TacoCommonCanvas/PanelList/FundsRunningLowPanel/Main/NoticeCash")
				.SetActive(true); 
				GameObject.Find("TacoGame/TacoCommonCanvas/PanelList/FundsRunningLowPanel/Main/NoticeTT")
				.SetActive(false); 
				GameObject.Find("TacoGame/TacoCommonCanvas/PanelList/FundsRunningLowPanel/Main/NoticeFundsCash")
				.SetActive(true); 
				GameObject.Find("TacoGame/TacoCommonCanvas/PanelList/FundsRunningLowPanel/Main/Buttons/DepositCash")
				.SetActive(true); 
				GameObject.Find("TacoGame/TacoCommonCanvas/PanelList/FundsRunningLowPanel/Main/NoticeFundsToken")
				.SetActive(false);
				GameObject.Find("TacoGame/TacoCommonCanvas/PanelList/FundsRunningLowPanel/Main/Buttons/PurchaseToken")
				.SetActive(false); 
			}
			else
			{
		...
		public void SetAllTournamentPanels(bool active) {
			fundsRunningLowPanel.SetActive(active);
		...
		public void TappedGameFromList(Tournament t) {
			...
			if (t.played) {
				...
				if (typeCurrency == 0) {
					replacedString = TacoConfig.TacoSurePlayModalBody.Replace("&prize", TacoManager.FormatCash(prize));
				}
				else {
		...
		public void CreateTournament(Tournament tourament) {
			...
			if (tourament.accessType == "public") {
				...
				Debug.Log("invitedFriendsStringCount : " + tourament.invitedEmails.Length);
				for (int i = 0; i < tourament.invitedEmails.Length; i++) {
					if(tourament.invitedEmails[i] != String.Empty)
					{
						invitedFriendsString = invitedFriendsString + "," + tourament.invitedEmails[i];
					}
				}
				Debug.Log("invitedFriendsString: " + invitedFriendsString);
				accessType = "private";
			}
		...
		public void OpenModalDepositDisablePanel() {
			TacoManager.OpenModalDepositDisablePanel();
		}

		public void OpenModalFundsRunningLowPanel() {
			TacoManager.OpenModalFundsRunningLowPanel();
		}
		...
		#region Join Tournament Play Again
		public void JoinPlayAgain(Tournament tour,  JoinTournamentResult tr) {
			TacoManager.OpenMessage(TacoConfig.TacoPublicJoining);
			var r = tour;
			Debug.Log("Join Tour ID: " + r.id);
			TacoManager.SetTarget(r);

			TacoManager.OpenModalJoinSuccessPanel(r, TacoConfig.JoinSuccessNotice);
			double val = 0;
			if (r.typeCurrency == 0) {
				//real money
				if (double.TryParse(tr.userFunds, out val)) {
					TacoManager.UpdateFundsWithToken(val, tr.currencyValue, tr.ticket, tr.userBonus);
				}
			}
			else {
				if (double.TryParse(tr.currencyValue, out val)) {
					TacoManager.UpdateFundsWithToken(val, tr.userFunds, tr.ticket);
				}
			}
			GameTaco.BalanceManager.Instance.LoadCurrentBalance();
